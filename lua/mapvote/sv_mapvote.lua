...
function MapVote.Start(length, current, limit, prefix)
	...
	local totals = (file.Exists(MapVote.DataPath, "DATA") and util.JSONToTable(file.Read(MapVote.DataPath)) or nil)
	
	local totalCount = totals and tonumber((totals.TotalChanged or 0)) or nil
	
	local maxCount = 0
	
	if (istable(totals)) then
		for _, v in pairs(totals) do
			if istable(v) then
				maxCount = math.Max(v.Count, maxCount)
			end
		end
	end
    ...
    for k, map in RandomPairs(maps) do
        ...
        if is_expression then
            if(string.find(map, prefix)) then -- This might work (from gamemode.txt)
                vote_maps[#vote_maps + 1] = {Map = map:sub(1, -5), Ratio = 0}
                amt = amt + 1
            end
        else
            for k, v in pairs(prefix) do
                if string.find(map, "^"..v) then
					if (totals and totals[map:sub(1, -5)]) then
						local curMapChanged = tonumber((totals[map:sub(1, -5)].Count or 0))
						
						if totalCount > 0 and curMapChanged / totalCount >= 0.7 then
							break
						end
					end
					...
					if (totals and totals[map:sub(1, -5)]) then
						...
						if curMapChanged > 0 then
							ratio = curMapChanged / maxCount
						end
					end
					
                    vote_maps[#vote_maps + 1] = {Map = map:sub(1, -5), Ratio = ratio}
                    amt = amt + 1
                    break
                end
            end
    ...
    net.Start("RAM_MapVoteStart")
        ...
        for i = 1, #vote_maps do
            net.WriteString(vote_maps[i].Map)
			net.WriteFloat(vote_maps[i].Ratio or 0)
        end
    ...
    timer.Create("RAM_MapVote", length, 1, function()
        ...
        timer.Simple(4, function()
            hook.Run("MapVoteChange", map.Map)
            RunConsoleCommand("changelevel", map.Map)
        end)
    end)
end
